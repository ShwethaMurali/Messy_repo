...
def compare_calls(bedlist1,bedlist2):
	...
	for bin in bedlist1: #0,1,2,..6..num_bins
		...
		if exists ==1:
			...
			bedtools = "bedtools"  			
			bedtools_check =  os.system("{0} intersect -wa -wb -a {1} -b {2} -f {4} -r > {3} "\
					.format(bedtools,bedfile1.name,bedfile2.name,results.name,pct_ovl))
			if bedtools_check !=0: 
				print "Bedtools not found!\nExiting.Please ensure you have BEDtools in your environment! "
				sys.exit(0)
